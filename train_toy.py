...
from utils import *
from utils.toy_loader import iterate_data, sample_test_data
...
parser.add_argument('--finetune', type=bool, nargs='?', default=False,
                    help='set the flag to True if finetune')
parser.add_argument('--pretrain_path', type=str, nargs='?', default='default',
                    help='set pretrain model path')
...
log_dir = os.path.join('./log_toy', args.tag)
save_model_dir = os.path.join('./save_model_toy', args.tag)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            if args.finetune:
                ...
                model.saver.restore(
                    sess, tf.train.latest_checkpoint(args.pretrain_path))
