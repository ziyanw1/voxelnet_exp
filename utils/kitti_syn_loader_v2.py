...
class Processor:
    def __init__(self, data_tag, f_rgb, f_lidar, f_obj, f_label, f_feat, data_dir, aug, is_testset):
    ...
    def __call__(self,load_index):
        if self.aug:
            ...
            ## FIXME:load feature here
            # reshape feature vector to [1, 1, chn]
...
def iterate_data(data_dir, shuffle=False, aug=False, is_testset=False, batch_size=1, multi_gpu_sum=1):
    ...
    f_feat = glob.glob(os.path.join(data_dir, 'feature', '*.npy'))
    f_rgb.sort()
    f_lidar.sort()
    f_label.sort()
    f_obj.sort()
    ...
    assert len(data_tag) == len(f_rgb) == len(f_lidar) == len(f_obj) ==len(f_label), "dataset folder is not correct"
    ...
    proc=Processor(data_tag, f_rgb, f_lidar, f_obj, f_label, f_feat, data_dir, aug, is_testset)
    ...
    for batch_idx in range(num_batches):
        ...
        template_feats = [ ret[6] for ret in rets ]
...
def sample_test_data(data_dir, batch_size=1, multi_gpu_sum=1):
    ...
    proc_val=Processor(data_tag, f_rgb, f_lidar, f_obj, f_label, data_dir, False, False)
