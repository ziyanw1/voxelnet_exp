...
from model.rpn_v2 import MiddleAndRPN


class RPN3DV2(object):
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            for idx, dev in enumerate(self.avail_gpus):
                with tf.device('/gpu:{}'.format(dev)), tf.name_scope('gpu_{}'.format(dev)):
                    ...
                    #input_rpn = tf.concat([feature.outputs, tf.tile(self.template_feature, [1, cfg.INPUT_HEIGHT,
                    #    cfg.INPUT_WIDTH, 1])], axis=-1)
                    rpn = MiddleAndRPN(
                        input=feature.feature, alpha=self.alpha, beta=self.beta, training=self.is_train)
                    ...
                    self.template_feature.append(rpn.template_feature)
    ...
    def train_step(self, session, data, train=False, summary=False):
        ...
        for idx in range(len(self.avail_gpus)):
            ...
            input_feed[self.template_feature[idx]] = template_feature[idx]
