...
class MiddleAndRPN:
    def __init__(self, input, alpha=1.5, beta=1, sigma=3, training=True, name=''):
        ...
        self.template_feature = tf.placeholder(tf,float32, [None, 1, 1, cfg.feature_channels], name='template_feature')
        self.input = tf.concat([input, tf.tile(self.template_feature, [1, cfg.INPUT_HEIGHT, cfg.INPUT_WIDTH, 1])],
            axis=-1)
